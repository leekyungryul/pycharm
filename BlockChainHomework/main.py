...
# 각각의 발생가능한 오류에 대해서 Exception을 상속받아서 class를 생성해주었습니다.
class OutOfFirstIdxErr(Exception):
    def __init__(self):
        super().__init__('시작범위를 잘못 입력하셨습니다..')
class OutOfLastIdxErr(Exception):
    def __init__(self):
        super().__init__('종료범위를 잘못 입력하셨습니다.')
class NegativeNumErr(Exception):
    def __init__(self):
        super().__init__('음수를 입력하셨습니다.')
class ZeroErr(Exception):
    def __init__(self):
        super().__init__('범위는 1이상 입력하세요')
class TypeErr(Exception):
    def __init__(self):
        super().__init__('숫자를 입력하세요')
...
class myHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        # 데이터를 담을 빈 리스트를 하나 생성
        data = []  # response json data
        # self.path가 만약 /block/로 시작한다면 아래로
        if None != re.search('/block/*', self.path):
            # self.path가 만약 /block/getBlockData로 시작한다면 아래로
            if None != re.search('/block/getBlockData', self.path):
                ...
                # print(recordID)
                # print(self.path)
                # print(type((int(endPoint))))
                # print(type(int(endPoint2)))
               # TODO : http://localhost:8666/block/getBlockData?from=1&end=10 -> from, end 문자열 검사
                try:
                    # self.send_response(200)
                    # self.send_header('Content-type', 'application/json')
                    # self.end_headers()
                    # block변수에 readBlockchain함수로 불러와서 블록체인 목록을 담는다(객체로 담아낸다.)
                    ...
                    # 음수를 입력한 경우
                    elif (int(startPoint) < 0 or int(endPoint) < 0):
                        self.send_response(500)
                        self.send_header('Content-type', 'application/json')
                        self.end_headers()
                        raise NegativeNumErr
                    # 시작범위를 0으로 입력한 경우
                    elif (int(startPoint) == 0 or int(endPoint) == 0):
                        self.send_response(500)
                        self.send_header('Content-type', 'application/json')
                        ...
                        raise ZeroErr
                    # 시작범위가 종료범위를 벗어나는 경우
                    elif (int(startPoint) > int(endPoint)):
                        self.send_response(500)
                        self.send_header('Content-type', 'application/json')
                        self.end_headers()
                        raise OutOfFirstIdxErr
                    # 종료범위가 보유한 블록체인목록의 범위를 벗어나는 경우
                    elif (len(block)-1 < int(endPoint)):
                        self.send_response(500)
                        self.send_header('Content-type', 'application/json')
                        self.end_headers()
                        raise OutOfLastIdxErr
                    # 예외에 안걸리면 200ok로 정상동작시킨다.
                    else:
                        self.send_response(200)
                        self.send_header('Content-type', 'application/json')
                        self.end_headers()
                ...
                # 각각의 except에 대한 내용을 변수(e)에 담아서 출력
                    ...
                    # data.append("error")
                    # pass
                # 사용자가 지정하지 못한 except를 잡아준다.
                except:
                    self.send_response(500)
                    self.send_header('Content-type', 'application/json')
                    self.end_headers()
                    data.append("error")
                # 최종 data에 담겨있는 딕셔너리형의 파일을 문자형으로 변환해서 client 화면에 출력해준다.
                finally:
