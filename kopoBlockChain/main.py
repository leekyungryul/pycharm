...
# 블록의 속성(블록의 번호, 생성 시각등) 전체를 해시하는 함수
...
def writeBlockchain(blockchain):
    ...
    for block in blockchain:
        # 객체는 파일로 저장할수없다 그래서 각각을 쪼개서 문자열로 변환해서 리스트에 담는다.
...
# 현재의 블록체인 정보를 조회하는 함수
# 채굴완료시 기존 블록체인과 연결고리를 만들기 위해서 필요하다.
...
def mineNewBlock(difficulty=g_difficulty, blockchainPath=g_bcFileName):
    ...
    strTxData = getTxData()
    # 만약 거래내역이 없다면 채굴을 할 필요가 없다 그래서 프린트하고 끝
    if strTxData == '' :
...
# a browser
class MinusError(Exception):  # Exception을 상속받아서 새로운 예외를 만듦
    def __init__(self):
    ...
    def do_GET(self):
        ...
        if None != re.search('/block/*', self.path):
            ...
            #             data.append(block[i].__dict__)
                ...
                #     # self.send_header('Content-type', 'application/json')
                #     self.end_headers()
                #     self.wfile.write(bytes("IndexError", "utf-8"))
                #     return
